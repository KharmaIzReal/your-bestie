    ...
    return (
      <div className="flex flex-col h-screen pb-28">
        <header className="px-4 py-4 flex items-center gap-3 glass border-b border-white/60 sticky top-0 z-10">
          <button onClick={() => setOpenId(null)} className="w-10 h-10 rounded-full bg-card flex items-center justify-center shadow-soft"><ArrowLeft className="w-5 h-5" /></button>
          <img src={open.bestie.avatar} className="w-11 h-11 rounded-2xl bg-muted" />
        ...
        <div className="px-4 pb-4 flex gap-2 items-center">
          <input value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => e.key === "Enter" && send()} placeholder="Send a message..." className="flex-1 h-12 rounded-full bg-card border-2 border-border px-5 text-sm focus:border-primary focus:outline-none" />
          <button onClick={() => send()} className="w-12 h-12 rounded-full gradient-primary text-primary-foreground shadow-glow flex items-center justify-center hover:scale-105 transition-transform">
  ...
  return (
    <div className="px-5 pt-6 space-y-5 animate-fade-in">
      ...
      <div className="relative">
        ...
        <input placeholder="Search besties" className="w-full h-12 pl-11 pr-4 rounded-full bg-card border-2 border-border text-sm focus:border-primary focus:outline-none" />
      </div>
            ...
            <button key={m.id} onClick={() => setOpenId(m.id)} className="shrink-0 text-center">
              <div className="relative">
                <div className="p-0.5 rounded-2xl gradient-warm">
                  <img src={m.avatar} className="w-16 h-16 rounded-2xl bg-card" />
                </div>
              </div>
      ...
      <section className="space-y-2">
        ...
        {allChats.map(c => (
          <button key={c.bestie.id} onClick={() => setOpenId(c.bestie.id)} className="w-full bg-card rounded-3xl p-3.5 shadow-soft flex items-center gap-3 hover:shadow-card transition-all">
            <img src={c.bestie.avatar} className="w-14 h-14 rounded-2xl bg-muted" />
